...
# ⚠️ Google Street View API Key
GSV_API_KEY = "AIzaSyA5dK4SMNTHecNzfpSHt9wQcq2vTy2jvTo"

# 数据路径
STREET_CENTERLINE_SHP_PATH = "street/KENSINGTON.shp"
# 采样参数 - 只在交叉口采样
...
IMAGE_SIZE = "640x640"
IMAGE_FOV = 90  # 视野角度（度），窄角以减少畸变

# 输出目录
PROJECT_DIR = Path('./philly_streetscape_project/KENSINGTON')
...
# ============================================================================
# SECTION 3: 测试下载 - 前2个路口（8张图）
# ============================================================================

TEST_N_INTERSECTIONS = 2
TEST_DIR_0   = OUTPUT_DIR / 'test_images_0deg'   # 正对路口
TEST_DIR_15  = OUTPUT_DIR / 'test_images_15deg'  # 水平偏转15度
TEST_DIR_0.mkdir(parents=True, exist_ok=True)
TEST_DIR_15.mkdir(parents=True, exist_ok=True)

print("\n" + "="*70)
print(f"STEP 3: Test Download - First {TEST_N_INTERSECTIONS} Intersections (GSV)")
print(f"测试下载前 {TEST_N_INTERSECTIONS} 个路口（0度 vs 15度水平偏转对比）")
print("="*70)

if len(sampling_points) == 0:
    print("\n  ⚠️  No sampling points available, skipping test download.")
else:
    first_n_ids = list(dict.fromkeys(
        p['intersection_idx'] for p in sampling_points
    ))[:TEST_N_INTERSECTIONS]
    test_points = [p for p in sampling_points if p['intersection_idx'] in first_n_ids]

    total_downloads = len(test_points) * 2  # 每个点下两张
    print(f"\n  Points: {len(test_points)}  ({TEST_N_INTERSECTIONS} intersections × {len(BEARINGS)} directions)")
    print(f"  Downloads: {total_downloads}  (×2 angles: 0° and 15°)")
    print(f"  0°  → {TEST_DIR_0.absolute()}")
    print(f"  15° → {TEST_DIR_15.absolute()}\n")

    GSV_BASE_URL = "https://maps.googleapis.com/maps/api/streetview"
    GSV_META_URL = GSV_BASE_URL + "/metadata"

    success, fail = 0, 0

    for pt in tqdm(test_points, desc="  Downloading"):
        lat, lon  = pt['lat'], pt['lon']
        base_bearing = pt['bearing']   # 正对路口方向
        point_id  = pt['point_id']

        # 1) 检查该位置是否有街景（只查一次）
        try:
            meta = requests.get(GSV_META_URL, params={
                'location': f'{lat},{lon}',
                'key': GSV_API_KEY
            }, timeout=10).json()
        except Exception as e:
            print(f"  ✗ {point_id}: metadata error - {e}")
            fail += 2
            continue

        if meta.get('status') != 'OK':
            print(f"  ✗ {point_id}: no GSV coverage (status={meta.get('status')})")
            fail += 2
            continue

        # 2) 下载两个角度
        for offset, save_dir in [(0, TEST_DIR_0), (15, TEST_DIR_15)]:
            heading = (base_bearing + offset) % 360
            try:
                resp = requests.get(GSV_BASE_URL, params={
                    'size': IMAGE_SIZE,
                    'location': f'{lat},{lon}',
                    'heading': heading,
                    'pitch': 0,
                ...
                if resp.status_code == 200 and len(resp.content) > 1000:
                    fname = save_dir / f"{point_id}_arm{pt['arm_direction']}_offset{offset}.jpg"
                    fname.write_bytes(resp.content)
                    success += 1
                else:
                    print(f"  ✗ {point_id} offset{offset}: bad response (status={resp.status_code})")
                    fail += 1
            except Exception as e:
                print(f"  ✗ {point_id} offset{offset}: {e}")
                fail += 1

            time.sleep(0.1)

    print(f"\n  ✓ Done!  Success: {success}  |  Failed: {fail}")
    print(f"  0°  images → {TEST_DIR_0.absolute()}")
    print(f"  15° images → {TEST_DIR_15.absolute()}")
    print(f"  Filename: pointID_armXXX_offsetYY.jpg")