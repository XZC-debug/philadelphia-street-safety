...
# 数据路径
STREET_CENTERLINE_SHP_PATH = "street/Center City.shp"
# 采样参数 - 只在交叉口采样
...
# 输出目录
PROJECT_DIR = Path('./philly_streetscape_project/Center City')
...
# ============================================================================
# SECTION 3: 下载所有路口街景（15度水平偏转）
# ============================================================================

DIR_15   = OUTPUT_DIR / 'images_15deg'
LOG_PATH = OUTPUT_DIR / 'download_log.csv'
DIR_15.mkdir(parents=True, exist_ok=True)

# 断点续传：读取已下载文件名
downloaded_15 = {f.stem for f in DIR_15.glob('*.jpg')}

# 读取已有日志（断点续传时追加）
log_records = []
if LOG_PATH.exists():
    log_records = pd.read_csv(LOG_PATH).to_dict('records')

print("\n" + "="*70)
print("STEP 3: Download All Intersections (GSV, 15° offset)")
print("下载所有路口街景（水平偏转15度）")
print("="*70)

if len(sampling_points) == 0:
    print("\n  ⚠️  No sampling points available, skipping download.")
else:
    print(f"\n  Total points:    {len(sampling_points):,}  ({len(sampling_points)//len(BEARINGS):,} intersections × {len(BEARINGS)} directions)")
    print(f"  Already done:    {len(downloaded_15):,}  (will skip)")
    print(f"  Images  → {DIR_15.absolute()}")
    ...
    for pt in tqdm(sampling_points, desc="  Downloading"):
        lat, lon         = pt['lat'], pt['lon']
        base_bearing     = pt['bearing']
        point_id         = pt['point_id']
        arm              = pt['arm_direction']
        intersection_idx = pt['intersection_idx']

        fname_stem = f"{point_id}_arm{arm}_offset15"

        # 断点续传：已存在则跳过
        if fname_stem in downloaded_15:
            success += 1
            continue
        ...
        try:
            ...
            if resp.status_code == 200 and len(resp.content) > 1000:
                fpath = DIR_15 / f"{fname_stem}.jpg"
                fpath.write_bytes(resp.content)
                success += 1

                # 记录坐标等信息
                log_records.append({
                    'filename':         fname_stem + '.jpg',
                    'point_id':         point_id,
                    ...
                    'heading':          heading,
                    'gsv_pano_id':      meta.get('pano_id', ''),
                    'gsv_date':         meta.get('date', ''),
                })
    ...
    print(f"  Images  → {DIR_15.absolute()}")
    print(f"  Log CSV → {LOG_PATH}")
    print(f"  Log columns: filename, point_id, lat, lon, arm_direction, heading, gsv_pano_id, gsv_date")